...
const footerMediaItems: FooterPopoverItem[] = [
		...
		<li>
			...
			<div
				className={cn(
					accordionPanelCls,
					isOpen
						? (panelClassName ?? accordionPanelOpenCls)
						: accordionPanelClosedCls,
				)}
			>
				{children}
			</div>
...
function QuickLinkRow({
	...
	items: { label: string; href: string; showDot?: boolean }[];
}) {
	return (
		<ul className="flex flex-wrap items-center gap-x-4 gap-y-1.5">
			{items.map((item) => (
				<li key={item.href}>
					<Link
						href={item.href}
						className="inline-flex items-center gap-x-1.5 n-book text-[13px] leading-[1.7] text-white transition-colors hover:text-white/80"
					>
						{item.showDot !== false && (
							<span
								className="inline-block shrink-0 w-[3px] h-[3px] rounded-full bg-white"
								aria-hidden
							/>
						)}
						{item.label}
							...
							<div className="w-full min-w-0">
								...
								<ul className={dropdownListCls}>
									{locations.map((city) => (
										...
										>
											<p className="fs-10 md:fs-12 py-3 text-white">
												{city.address}
											</p>
										</FooterAccordionItem>
									))}
							...
							<div className={footerBottomSectionCls}>
								...
								<div className="mt-2 flex flex-col gap-1.5 sm:gap-2">
									...
									<div className="flex flex-wrap items-center gap-x-4 gap-y-1.5 n-book text-[13px] leading-[1.7]">
										...
										<span
											className="inline-block h-[3px] w-[3px] shrink-0 rounded-full bg-white"
											aria-hidden
										/>
