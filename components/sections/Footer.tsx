...
function QuickLinkRow({ items }: { items: { label: string; href: string; showDot?: boolean }[] }) {
  return (
    <ul className="flex flex-wrap items-center gap-x-4 gap-y-1.5">
      {items.map((item) => (
        <li key={item.href}>
          ...
          >
            {item.showDot !== false && <span className="inline-block shrink-0 w-[3px] h-[3px] rounded-full bg-white" aria-hidden />}
            {item.label}
          </Link>
        </li>
      ))}
            ...
            >
              ...
              <div className={footerBottomSectionCls}>
                ...
                <div className="mt-2 flex flex-col gap-1.5 sm:gap-2">
                  ...
                  <QuickLinkRow
                    items={[
                      { label: "Global Networks", href: "/global-networks", showDot: false },
